...
function enqueue_ajax_file_upload_script() {
		...
		wp_localize_script('custom-script', 'ajax_obj', array(
			...
			'nonce'   => wp_create_nonce('ajax-file-upload-nonce'),
		));
}
add_action('wp_enqueue_scripts', 'enqueue_ajax_file_upload_script');
...
add_shortcode('file_upload_form', 'file_upload_form_shortcode');
	...
	function handle_file_upload() {
		...
		check_ajax_referer('ajax-file-upload-nonce', 'nonce');
    ...
    if (!empty($_FILES['file'])) {
        ...
        if ($movefile && !isset($movefile['error'])) {
            ...
            if ($post_id) {
                update_post_meta($post_id, 'cus_uploaded_image', $file_url);
                echo "File uploaded successfully. URL: " . $file_url;
            } else {
                echo "File uploaded, but no post ID provided.";
            }
        } else { 
